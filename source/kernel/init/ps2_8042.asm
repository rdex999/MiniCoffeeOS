...
%macro PS2_8042_CHECK_TWO_CHANNELS 0
  ...
  test byte [bp - 1], 0010_0000b                    ; Check if there is a second PS/2 port
  ...
  PS2_SEND_COMMAND PS2_CMD_READ_CONFIGURATION_BYTE
  test al, 0010_0000b                               ; Jump if there is only one PS/2 port
  jnz %%PS2_CHECK_TWO_CHANNELS_end
...
%%PS2_INTERFACE_TESTS_testFailed:
  PANIC_LM errPs2CtrlSelfTestFailed
...
%macro PS2_8042_ENABLE_PORTS 0
  ...
  test bl, 0010_0000b                           ; Check if there is a second PS/2 port
